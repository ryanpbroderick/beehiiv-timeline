...
# Initialize clients (with basic sanity checks)
if not SUPABASE_URL or not SUPABASE_KEY:
    raise RuntimeError("Missing SUPABASE_URL or SUPABASE_KEY env var(s).")
if not OPENAI_API_KEY:
    raise RuntimeError("Missing OPENAI_API_KEY env var.")
if not BEEHIIV_API_KEY or not BEEHIIV_PUBLICATION_ID:
    raise RuntimeError("Missing BEEHIIV_API_KEY or BEEHIIV_PUBLICATION_ID env var(s).")
...
def fetch_beehiiv_posts(limit: int = 100, page: int = 1) -> Dict:
    ...
    param_sets = [
        ...
        {"status": "confirmed", "limit": limit, "page": page},
    ]
...
def validate_cards(cards: List[Dict], clean_content: str) -> List[Dict]:
    ...
    for c in _safe_list(cards):
        if not isinstance(c, dict):
            dropped_count += 1
            print("⚠️  Dropped card: not a dict")
            continue
        ...
        confidence_raw = c.get("confidence", 0.75)
        try:
...
"""
    ...
    try:
        ...
        # Derive periods from then_start/then_end across cards
        period_ids: List[str] = []
        for c in cards:
        ...
        # Legacy UI pull quote: prefer first claim
...
def store_cards(beehiiv_id: str, publish_date: str, title: str, url: str, cards: List[Dict]) -> bool:
    """Delete + insert cards for an issue into a `cards` table."""
    try:
